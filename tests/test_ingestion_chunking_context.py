...
def test_short_docx_paragraphs_are_merged_with_source_range() -> None:
    upload = ValidatedUpload(
        file_name="lesson.docx",
        normalized_name="lesson.docx",
        title="Lesson",
        data=b"data",
    ...
    parsed = ParsedDocument(
        ...
        kind=DocumentKind.DOCX,
        blocks=(
            ParsedBlock(0, "Đoạn đầu tiên.", "Đoạn 1", section="Mục A"),
            ParsedBlock(1, "Đoạn thứ hai.", "Đoạn 2", section="Mục A"),
            ParsedBlock(2, "Đoạn thứ ba.", "Đoạn 3", section="Mục A"),
            ParsedBlock(3, "Nội dung mục mới.", "Đoạn 4", section="Mục B"),
        ),
        extracted_chars=59,
    )
    ...
    assert [chunk.source for chunk in chunks] == ["Đoạn 1–3", "Đoạn 4"]
    assert chunks[0].original_text == "Đoạn đầu tiên. Đoạn thứ hai. Đoạn thứ ba."
    assert [chunk.section for chunk in chunks] == ["Mục A", "Mục B"]


def test_docx_table_interrupts_paragraph_merging() -> None:
    upload = ValidatedUpload(
        file_name="lesson.docx",
        normalized_name="lesson.docx",
        title="Lesson",
    ...
    parsed = ParsedDocument(
        ...
        kind=DocumentKind.DOCX,
        blocks=(
            ParsedBlock(0, "Trước bảng.", "Đoạn 1", section="Mục A"),
            ParsedBlock(1, "Cột A | Cột B", "Bảng 1", section="Mục A"),
            ParsedBlock(2, "Sau bảng.", "Đoạn 2", section="Mục A"),
        ),
        extracted_chars=34,
    )
    ...
    assert [chunk.source for chunk in chunks] == ["Đoạn 1", "Bảng 1", "Đoạn 2"]


def test_docx_paragraph_groups_respect_chunk_limit_without_losing_text() -> None:
    upload = ValidatedUpload(
        file_name="lesson.docx",
        normalized_name="lesson.docx",
        title="Lesson",
    ...
    parsed = ParsedDocument(
        ...
        kind=DocumentKind.DOCX,
        blocks=tuple(
            ParsedBlock(index, text, f"Đoạn {index + 1}", section="Mục A")
            for index, text in enumerate(texts)
        ),
        extracted_chars=sum(map(len, texts)),
    )
    ...
    assert [chunk.source for chunk in chunks] == ["Đoạn 1–2", "Đoạn 3–4"]
    assert all(len(chunk.original_text) <= 25 for chunk in chunks)
    assert " ".join(chunk.original_text for chunk in chunks) == " ".join(texts)
